...
#'  for Y-LINKED inversions capturing the male-beneficial
#'  allele at a locus under SEXUALLY ANTAGONISTIC SELECTION
...
#' Y-linked SLR-expanding inversion capturing a male-beneficial 
#' SA allele w/ partially recessive deleterious mutations

##  EXACT RECURSIONS -- SA locus

qXf.prime  <-  function(hf, sf, qtXf, qtXm) {
	(qtXf + qtXm*(1 - hf*sf) - qtXf*sf*(hf + 2*qtXm*(1 - hf))) / 
	(2*(1 - sf*(qtXf*qtXm + hf*(qtXm + qtXf*(1 - 2*qtXm)))))
}

qXm.prime  <-  function(rSA, hm, sm, qtXf, qtY, YI.t) {
(qtXf*(1 - hm*sm*(1 - qtY)*(1 - YI.t) - rSA*(1 - hm*sm)*(1 - YI.t)) + qtY*rSA*(1 - hm*sm)*(1 - YI.t)) / 
	(1 - sm*(1 - YI.t*(1 - hm) - qtY*(1 - hm - YI.t*(1 - hm)) - qtXf*(1 - hm - qtY*(1 - 2*hm) - YI.t*(1 - 2*hm)*(1 - qtY))))
}

qY.prime  <-  function(rSA, hm, sm, qtXf, qtY) {
(qtY + rSA*(qtXf - qtY) - hm*sm*(qtXf*rSA + qtY*(1 - qtXf - rSA))) /
	(1 - sm*(1 - qtXf - qtY*(1 - hm - qtXf) + hm*qtXf*(1 - 2*qtY)))
}


##  EXACT RECURSIONS -- INVERSION
wBarY  <-  function(sm, hm, qtXf, qtY, n, r, s, h, YI.t, qt.I.wt, qt.Y.wt, qt.Y.del, XaOv.t.wt, XaOv.t.del) {
	pt.I.wt    <-  1 - qt.I.wt
	pt.Y.del   <-  1 - qt.Y.del
	pt.Y.wt    <-  1 - qt.Y.wt
...
YI.prime  <-  function(sm, hm, qtXf, qtY, n, r, s, h, YI.t, qt.I.wt, qt.Y.wt, qt.Y.del, XaOv.t.wt, XaOv.t.del, ...) {
	(YI.t*(1 - sm*hm*(1 - qtXf))*(( 1 - s*(h*(1 - XaOv.t.del) + XaOv.t.del) )^r) * (1 - s*(h*((1 - qt.I.wt)*XaOv.t.wt + qt.I.wt*(1 - XaOv.t.wt)) + qt.I.wt*XaOv.t.wt))^(n-r)) / wBarY(sm=sm, hm=hm, qtXf=qtXf, qtY=qtY, n=n, r=r, s=s, h=h, YI.t=YI.t, qt.I.wt=qt.I.wt, qt.Y.wt=qt.Y.wt, qt.Y.del=qt.Y.del, XaOv.t.wt=XaOv.t.wt, XaOv.t.del=XaOv.t.del)
}


##  EXACT RECURSIONS -- DELETERIOUS ALLELES AT SELECTED LOCI
...
findEq  <- function(sf, hf, sm, hm, qtXf, qtY, rSA, r, h, s, n, u, qHat, qHatDel) {
	...
	YI.t[1]        <-  round(YI.prime(sm=sm, hm=hm, qtXf=qtXfHat, qtY=qtYHat, n=n, r=r, s=s, h=h, YI.t=0, qt.I.wt=0, qt.Y.wt=qHat, qt.Y.del=qHatDel, XaOv.t.wt=qHat, XaOv.t.del=qHatDel), digits=9)
	qXf.t[1]       <-  round(qXf.prime(hf=hf, sf=sf, qtXf=qtXfHat, qtXm=qtXmHat), digits=9)
	qXm.t[1]       <-  round(qXm.prime(rSA=rSA, hm=hm, sm=sm, qtXf=qtXfHat, qtY=qtYHat, YI.t=0), digits=9)
	qY.t[1]        <-  round(qY.prime(rSA=rSA, hm=hm, sm=sm, qtXf=qtXfHat, qtY=qtYHat), digits=9)
	...
	diff  <-  1
	while(diff > 1e-8) {
		YI.t[i]        <-  round(YI.prime(sm=sm, hm=hm, qtXf=qXf.t[i-1], qtY=qY.t[i-1], n=n, r=r, s=s, h=h, YI.t=YI.t[i-1], qt.I.wt=qI.wt.t[i-1], qt.Y.wt=qY.wt.t[i-1], qt.Y.del=qY.del.t[i-1], XaOv.t.wt=XaOv.wt.t[i-1], XaOv.t.del=XaOv.del.t[i-1]), digits=9)
		qXf.t[i]       <-  round(qXf.prime(hf=hf, sf=sf, qtXf=qXf.t[i-1], qtXm=qXm.t[i-1]), digits=9)
		qXm.t[i]       <-  round(qXm.prime(rSA=rSA, hm=hm, sm=sm, qtXf=qXf.t[i-1], qtY=qY.t[i-1], YI.t=0), digits=9)
		qY.t[i]        <-  round(qY.prime(rSA=rSA, hm=hm, sm=sm, qtXf=qXf.t[i-1], qtY=qY.t[i-1]), digits=9)
...
######################################################
#' Equilibrium SA allele frequency (prior to inversion) 
#' WHEN SA LOCUS IS UNLINKED WITH SLR
SA_qHat_Add  <-  function(sf, sm) {
	qHat  <-  (sf - sm*(1 - sf)) / (2*sm*sf)
	if(qHat < 0) {
		qHat  <-  0
	}
	if(qHat > 1) {
...
SA_qHat_DomRev  <-  function(h, sf, sm) {
	qHat  <-  (2*sf - (sf + sm)*(2*h)) / (2*(1 - 2*h)*(sf + sm))
	if(qHat < 0) {
		qHat  <-  0
	}
	if(qHat > 1) {
...
######################################################
#' Deterministic overall selection coefficient for 
#' inversion, sI_SA ... based on eq. frequencies.
sI_SA  <-  function(sm, hm, Yhat, Xfhat) {
    (sm*(1 - Yhat)*(1 - Xfhat - hm*(1 - 2*Xfhat))) / (1 - sm*(1 - Xfhat - Yhat*(1 - hm - Xfhat) + hm*Xfhat*(1 - 2*Yhat)))
}

sI_SA_add  <-  function(sm, Yhat, Xfhat) {
    (2*sm*Yhat*(1 - Yhat)) / (2 - sm*(2 - Xfhat - Yhat))
}

###########################
#' Generate WF simulation data
#' for different length inversions, 
#' with different deleterious mutation loads

makeDeterministicFigSimData  <-  function(sf, hf, sm, hm, rSA=1/2, r = 1, x = 0.2, h = 0.1, s = 0.01, Ufactor = 2, generations = 10^4, ...) {
	# Parameters
	U     <-  Ufactor*s
	nTot  <-  10^4
	u     <-  U/nTot
	...
	# Get Equilibrium SA allele frequencies prior to inversion
	if(hm != hf) {
		stop('Error: male/female SA dominance coeffs must be equal')
	}
	if(rSA > 1/2) {
		stop('Error: recombination rate > 1/2!')
	}
	if(rSA == 1/2) {
		if(hf == 1/2) {
			qHat.SA.a  <-  SA_qHat_Add	(sf=sf, sm=sm)
		}
		if(hf < 1/2) {
			qHat.SA.a  <-  SA_qHat_DomRev(sf=sf, sm=sm, h=hf)
		}
	}
	if(rSA < 1/2) {
						qXf.t       <-  eq.df$Xf
	...
	# Find equilibrium prior to inversion 
	eqs  <-  findEq(sI=sI, rSA=rSA, r=r, h=h, s=s, n=n, u=u, qHat=qHat.SA.a, qHatDel=qHatDel)
	
	# First generation (all loci at equilibrium frequencies when inversion arises)
	YI.t[1]        <-  round(YI.prime(sm=sm, hm=hm, qtXf=eqs$qXf.init, qtY=eqs$qY.init, n=n, r=r, s=s, h=h, YI.t=0, qt.I.wt=0, qt.Y.wt=eqs$qY.wt.init, qt.Y.del=eqs$qY.del.init, XaOv.t.wt=eqs$XaOv.wt.init, XaOv.t.del=eqs$XaOv.del.init), digits=9)
	qXf.t[1]       <-  round(qXf.prime(hf=hf, sf=sf, qtXf=eqs$qXf.init, qtXm=eqs$qXm.init), digits=9)
	qXm.t[1]       <-  round(qXm.prime(rSA=rSA, hm=hm, sm=sm, qtXf=eqs$qXf.init, qtY=eqs$qY.init, YI.t=0), digits=9)
	qY.t[1]        <-  round(qY.prime(rSA=rSA, hm=hm, sm=sm, qtXf=eqs$qXf.init, qtY=eqs$qY.init), digits=9)
	...
	while(i < generations+1) {
		YI.t[i]        <-  round(YI.prime(sm=sm, hm=hm, qtXf=qXf.t[i-1], qtY=qY.t[i-1], n=n, r=r, s=s, h=h, YI.t=YI.t[i-1], qt.I.wt=qI.wt.t[i-1], qt.Y.wt=qY.wt.t[i-1], qt.Y.del=qY.del.t[i-1], XaOv.t.wt=XaOv.wt.t[i-1], XaOv.t.del=XaOv.del.t[i-1]), digits=9)
		qXf.t[i]       <-  round(qXf.prime(hf=hf, sf=sf, qtXf=qXf.t[i-1], qtXm=qXm.t[i-1]), digits=9)
		qXm.t[i]       <-  round(qXm.prime(rSA=rSA, hm=hm, sm=sm, qtXf=qXf.t[i-1], qtY=qY.t[i-1], YI.t=YI.t[i-1]), digits=9)
		qY.t[i]        <-  round(qY.prime(rSA=rSA, hm=hm, sm=sm, qtXf=qXf.t[i-1], qtY=qY.t[i-1]), digits=9)
		XaOv.wt.t[i]   <-  round(XaOv.wt.prime(u=u, s=s, h=h, XaOv.t.wt=XaOv.wt.t[i-1], XaSp.t.wt=XaSp.wt.t[i-1]), digits=9)
		XaSp.wt.t[i]   <-  round(XaSp.wt.prime(u=u, s=s, h=h, YI.t=YI.t[i-1], qt.I.wt=qI.wt.t[i-1], qt.Y.wt=qY.wt.t[i-1], XaOv.t.wt=XaOv.wt.t[i-1]), digits=9)
		XaOv.del.t[i]  <-  round(XaOv.del.prime(u=u, s=s, h=h, XaOv.t.del=XaOv.del.t[i-1], XaSp.t.del=XaSp.del.t[i-1]), digits=9)
	...
	YI.t        <-  c(YI.0,YI.t)
	qXf.t       <-  c(eqs$qXf.init, qXf.t)
	qXm.t       <-  c(eqs$qXm.init, qXm.t)
	qY.t        <-  c(eqs$qY.init, qY.t)
...
## 
wBarY.multi  <-  function(sm, hm, qtXf, qtY, n, r, s, h, YI.t, qt.I.wt, qt.Y.wt, qt.Y.del, XaOv.t.wt, XaOv.t.del) {
	pt.I.wt    <-  1 - qt.I.wt
	pt.Y.del   <-  1 - qt.Y.del
	pt.Y.wt    <-  1 - qt.Y.wt
		   YI.t*(1 - sm*hm*(1 - qtXf))*(prod( 1 - s*(h*(1 - XaOv.t.del) + XaOv.t.del) )) * prod(1 - s*(h*(pt.I.wt*XaOv.t.wt + qt.I.wt*(1 - XaOv.t.wt)) + qt.I.wt*XaOv.t.wt)) +
	(1 - YI.t)*(1 - sm*(hm*((1 - qtXf)*qtY + qtXf*(1 - qtY)) + (1 - qtXf)*(1 - qtY)))*( prod(1 - s*(h*((1 - XaOv.t.del)*qt.Y.del + XaOv.t.del*pt.Y.del) + XaOv.t.del*qt.Y.del)) * prod((1 - s*(h*((1 - XaOv.t.wt)*qt.Y.wt + XaOv.t.wt*pt.Y.wt) + XaOv.t.wt*qt.Y.wt))) )
}

YI.multi.prime  <-  function(sm, hm, qtXf, qtY, n, r, s, h, YI.t, qt.I.wt, qt.Y.wt, qt.Y.del, XaOv.t.wt, XaOv.t.del, ...) {
	(YI.t*(1 - sm*hm*(1 - qtXf))*(prod( 1 - s*(h*(1 - XaOv.t.del) + XaOv.t.del))) * prod(1 - s*(h*((1 - qt.I.wt)*XaOv.t.wt + qt.I.wt*(1 - XaOv.t.wt)) + qt.I.wt*XaOv.t.wt))) / wBarY.multi(sm=sm, hm=hm, qtXf=qtXf, qtY=qtY, n=n, r=r, s=s, h=h, YI.t=YI.t, qt.I.wt=qt.I.wt, qt.Y.wt=qt.Y.wt, qt.Y.del=qt.Y.del, XaOv.t.wt=XaOv.t.wt, XaOv.t.del=XaOv.t.del)
}
...
# text(x=(N/2), y=0.9, labels=paste(l, sep=''))
					...
					while(YI.t*(1 - YI.t) > 0) {
						...
						qXf.t       <-  round(qXf.prime(hf=hf, sf=sf, qtXf=qXf.t, qtXm=qXm.t), digits=9)
						qXm.t       <-  round(qXm.prime(r=rSA, hm=hm, sm=sm, qtXf=qXf.t, qtY=qY.t, YI.t=YI.t), digits=9)
						qY.t        <-  round(qY.prime(r=rSA, hm=hm, sm=sm, qtXf=qXf.t, qtY=qY.t), digits=9)
						...
						YI.sel      <-  round(YI.prime(sm=sm, hm=hm, qtXf=qXf.t, qtY=qY.t, n=n, r=r, s=s, h=h, YI.t=YI.t, qt.I.wt=qI.wt.t, qt.Y.wt=qY.wt.t, qt.Y.del=qY.del.t, XaOv.t.wt=XaOv.wt.t, XaOv.t.del=XaOv.del.t), digits=9)
...
#t=t+1
	...
	filename  <-  paste("./output/data/simResults/SA_PrFixFig_rSA",rSA,"_sf", sf,"_sm", sm,"_hSA", hf, "_h", h, "_s", s, Nfname, ".csv", sep="")
	d  <-  data.frame(
										"rSA"    =  rep(rSA, times=length(PrFix)),
										"sf"     =  rep(sf, times=length(PrFix)),
										"sm"     =  rep(sm, times=length(PrFix)),
										"hSA"    =  rep(hf, times=length(PrFix)),
